...
#function count_digits_left_of_decimal(x::Number)
#    int_part = floor(Int, x)
#    return length(string(int_part))
#end

#df_lin.HowHuge = count_digits_left_of_decimal.(df_lin.Level)
#df_lin.StartLevel = df_lin.Level
...
df_lsi = innerjoin(df_lsi, select(df_li, [:LocationId, :IngredientId, :Index_LI, :Index_I]), on = [:LocationId, :IngredientId])
...
#-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-
# LI_2 LSI_2 LS_3 LI_2 LSI_2 LS_3 LI_2 LSI_2 LS_3 LI_2 LSI_2 LS_3 LI_2 LSI_2 LS_3 LI_2 LSI_2 LS_3 
...
#has_vol_nut_0 = df_ls[df_ls.HasVolNut .== 0, [:LocationId, :SpecificationId]]
#df_lsi.Volume[in(Set(has_vol_nut_0.LocationId .* has_vol_nut_0.SpecificationId))] .= 1
...
for a in 1:length(indexLsn_ratio)
    ...
    q_lsn_top = lsn_top_ids[lsn]
    q_lsn_bot = lsn_bot_ids[lsn]
    top_sum = sum_lsn[q_lsn_top]
    bot_sum = sum_lsn[q_lsn_bot]
    # a/b > c --> a > bc --> a - bc > 0 --> a - bc + e1 > 0
    # a/b < d --> a < bd --> a - bd < 0 --> a - bd - e2 < 0
    if lsn_has_min[lsn] == 1
        @constraint(CNC, top_sum - bot_sum * lsn_min[lsn] + x_lsn_neg[lsn] >= 0.0)
    end
    if lsn_has_max[lsn] == 1
        @constraint(CNC, top_sum - bot_sum * lsn_max[lsn] - x_lsn_pos[lsn] <= 0.0)
    end
...
for i in 1:cnt_i
    ...
    if !ismissing(i_2_li[i][1])
        @constraint(CNC, sum(x_li[i_2_li[i][n]] for n in 1:length(i_2_li[i]) if li_global[i_2_li[i][n]] == 0) == sum_i[i])
    else
...
obj = (tot_cost + tot_cost_zero + tot_ls_bad * LS_BAD + tot_i_bad * I_BAD + tot_li_bad * LI_BAD + tot_lsn_bad_n_rat * LSN_BAD * (1.0) + tot_lsn_bad_y_rat * LSN_BAD * (1.0)) / OBJ_SCALAR
...
# Define structures to store the LP details
#a_format = 1 # Example format
num_col = HiGHS.Highs_getNumCols(h)
num_row = HiGHS.Highs_getNumRows(h)
num_nz = HiGHS.Highs_getNumNz(h)
#sense = 1 # Sense of the optimization (1 for minimization, -1 for maximization)
#offset = 0.0  # Objective offset
#col_cost = Vector{Float64}(undef, HiGHS.Highs_getNumCols(h))
#col_lower = Vector{Float64}(undef, HiGHS.Highs_getNumCols(h))
#col_upper = Vector{Float64}(undef, HiGHS.Highs_getNumCols(h))
#row_lower = Vector{Float64}(undef, HiGHS.Highs_getNumRows(h))
#row_upper = Vector{Float64}(undef, HiGHS.Highs_getNumRows(h))
#a_start = Vector{Int32}(undef, HiGHS.Highs_getNumCols(h))
#a_index = Vector{Int32}(undef, HiGHS.Highs_getNumNz(h))
#a_value = Vector{Float64}(undef, HiGHS.Highs_getNumNz(h))
#integrality = Vector{Int32}(undef, HiGHS.Highs_getNumCols(h))

# Retrieve the LP details
#HiGHS.Highs_getLp(h, a_format, num_col, num_row, num_nz, sense, offset, col_cost, col_lower, col_upper, row_lower, row_upper, a_start, a_index, a_value, integrality)
...
for a in 1:length(set_set)
    ...
    if set_set[a] .!= "ls"
        transform!(df, [:Level, :Min] => ByRow(==) => :Status)
        df.Status = ifelse.(df.Status, "min", "")
        transform!(df, [:Level, :Min] => ByRow(<) => :Status)
        df.Status = ifelse.(df.Status, "below", "")
    end
end
